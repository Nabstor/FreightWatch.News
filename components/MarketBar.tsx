...
export function MarketBar() {
  ...
  useEffect(() => {
    fetch('/api/markets')
      .then(r => r.json())
      .then(data => { setFuel(data.fuel || []); setLive(true); })
      .catch(() => {});
  }, []);

  // Always render at fixed height — prevents layout shift when data arrives
  return (
    <div style={{ borderBottom: '1px solid #e2e2e2', background: '#fafafa', height: '40px' }}>
      <div className="max-w-screen-xl mx-auto px-4 md:px-6" style={{ height: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', overflowX: 'auto', height: '100%' }}>
          {fuel.length > 0 ? (
            ...
            <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '9px', color: '#ccc', letterSpacing: '0.08em' }}>
              FUEL &amp; OIL
            </span>
          )}
        </div>
